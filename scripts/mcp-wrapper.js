 ...
 * 
 * Wrapper script for Gendl MCP integration with:
...
function startGendlContainer() {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      logger.info(`Preparing to start Gendl container using image ${dockerImage}`);
      ...
      const containerName = `gendl-mcp-${new Date().getTime()}`;
      ...
      gendlContainerName = containerName;
      logger.info(`Started Gendl container with name: ${containerName}`);
      ...
      setTimeout(() => {
        ...
        exec(`docker ps --filter "name=${containerName}" --format "{{.ID}}"`, (error, stdout, stderr) => {
          ...
          if (stdout.trim()) {
            ...
            logger.info(`Container ${containerName} is running, waiting for Gendl server to start`);
            ...
            setTimeout(() => {
              exec(`docker ps --filter "name=${containerName}" --format "{{.ID}}"`, (error, stdout, stderr) => {
                ...
                if (stdout.trim()) {
                  logger.info(`Container ${containerName} is now running, waiting for Gendl server to start`);
                  waitForGendlServer(30)
                    .then(() => resolve(true))
                    .catch(error => reject(error));
                } else {
...
function handlePingGendl(request) {
  ...
  makeHttpRequest(options, null, (error, response) => {
    if (error) {
      logger.error(`Error pinging Gendl server: ${error.message}`);
