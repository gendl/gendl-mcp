...
/*
 * mcp-format-wrapper-fixed-v2.js
 * 
 * This is an improved version with better handling for lisp_eval responses specifically for Claude
 */
...
log('Starting improved Gendl MCP wrapper with better lisp_eval handling for Claude');
  ...
  switch (toolName) {
    case 'ping_gendl':
      makeGendlRequest(`${GENDL_BASE_PATH}/claude/ping`)
        .then(data => {
          // Always return as text for ping
          const response = {
            jsonrpc: '2.0',
            id: request.id,
            result: {
    ...
    case 'list_objects':
      makeGendlRequest(`${GENDL_BASE_PATH}/objects`)
        .then(data => {
          try {
            // For list_objects, format the output as text
            ...
            if (jsonData && Array.isArray(jsonData)) {
              ...
              objectsList += "No objects found or unexpected response format.";
            }
        ...
        .then(data => {
          try {
            ...
            if (jsonData.success) {
              ...
              // Always format result as plain text
              const resultText = jsonData.result ? jsonData.result.toString() : "Evaluation successful (no return value)";
...
log('Improved Gendl MCP wrapper initialized - ready to handle requests');
