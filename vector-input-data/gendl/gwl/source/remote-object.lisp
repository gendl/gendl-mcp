...
(defun change-case (object function)
  (typecase object
    (keyword (make-keyword (funcall function (string object))))
    (symbol (cond ((null object) nil)
                  (t (let* ((name (funcall function (symbol-name object)))
                            (package (symbol-package object)))
                       (intern name package)))))
    (list (mapcar (lambda (item) (change-case item function)) object))
    (t object)))
    ...
    (let ((plist (list :message (make-keyword message)
      ...
      (let ((result (the (remote-execute "send-remote-message" plist))))
        ...
        (cond ((and (consp result) (eql (first result) :error)
               ...
               (progn
                 (when *warnings?* (warn "~&Remote object returned error, creating a new one...~%"))
...
(defun decode-from-http (item)
  (if (and (consp item) (not (metatilities:dotted-pair-p item)))
      (multiple-value-bind (object-type initargs)
          (destructure-object-from-http item)
...
(defun encode-plist-args (plist)
  ...
  (let ((*print-case* :downcase))
    (when plist
      (cons (first plist)
	    (cons (encode-for-http (second plist))
                  (encode-plist-args (rest (rest plist))))))))
...
(defun gwl-host ()
  ...
  (when (boundp '*http-server*)
    (let ((sock (wserver-socket *http-server*)))
