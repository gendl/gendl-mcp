...
(let ((enable-compression? (and (member :zlib-deflate *features*) t)))
  (setq *http-server* (make-instance 'wserver :enable-compression enable-compression?)
        *https-server* (make-instance 'wserver :enable-compression enable-compression?)))

(defparameter *start-http?* t)
(defparameter *start-https?* nil)
...
(defparameter *default-ports* (list :http 80 :https 443))

(defparameter *publish-directory-headers* (list (cons :cache-control "public, max-age=3600")))

#+allegro (defparameter *ansify-plists?* t)

(defvar *incoming-remote-to-case*  nil)

(defparameter *http-server-external-format*
  #+(and windows allegro) :utf8-base
  #-(and windows allegro) uiop:*utf-8-external-format*)
