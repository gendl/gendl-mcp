...
Default is nil." 
  ...
  ((raw-rows (if (the key-row?) (rest (the default)) (the default)))
   ...
   (raw-row-array (make-array (length (the raw-rows)) :initial-contents (the raw-rows)))
...
(define-object grid-row (skeleton-form-control)
  ...
  :computed-slots ((default-array (make-array (length (the default)) :initial-contents (the default)))
