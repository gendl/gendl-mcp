...
The tolerance for the operation is controlled by the brep's tolerance.")
  ...
  ("GDL Brep object. The brep to be offset."
   ...
   ("Boolean. `nil` (the default) results in filleted edges. `t` will
extend the faces to make sharp edges. Default `nil`."
    extended-offset? nil)

   ("Boolean. If the offset result is smaller than the input, this
controls whether self-intersecting faces are trimmed. Default `nil`."
    self-intersect? nil))
  ...
  ((%actual-brep% 
   ...
   (%native-brep% 
    (make-offset-brep *geometry-kernel* 
		      ...
		      :extended-offset-p (if (the extended-offset?) 1 0)
		      :self-intersect-p (if (the self-intersect?) 1 0)))))
