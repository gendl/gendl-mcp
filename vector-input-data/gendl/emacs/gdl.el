...
(defun configure-slime ()
  ...
  ;;(print (concat "*gendl-home* is: "  *gendl-home*))

  ;;
  ;; FLAG -- shouldn't this be loading slime-helper.el before the configurations? 
  ;;
  (let ((built-in-config (concat *gendl-home* "emacs/configure-glime.el"))
    ...
    (let ((config-file (if (file-exists-p built-in-config) built-in-config home-config)))
      ...
      (unless quicklisp-slime-helper
	(setq quicklisp-slime-helper (file-truename (let ((ql-directory (concat *gendl-home* "../quicklisp/")))
					              (concat ql-directory "slime-helper.el")))))
	  ...
	  (load-file quicklisp-slime-helper)
        ;;
        ;; else set up path to slime manually, assume gendl directory exists.
        ;;
        (let ((slime-dir (concat *gendl-home* "/docker/slime/")))
          (add-to-list 'load-path slime-dir)
          (require 'slime-autoloads)
          (setq slime-backend (expand-file-name "swank-loader.lisp" slime-dir))
...
(defun slime-init-command (port-filename _coding-system)
  ...
  (format "%S\n\n"
          `(progn
             ;;(gendl::initialize)
             ;;(glisp:load-swank :from-emacs? t)
             (swank:start-server ,(slime-to-lisp-filename port-filename)))))
...
(setq gdl-startup-string "(gendl::startup-banner)(gdl:start-gdl!)")
...
(defun prior-to-glime ()
  ...
  (read-only-mode 1) ;; (toggle-read-only)
...
(when (file-exists-p *eli-init*)
  ...
  (defun get-executable (exe-name)
	...
	(replace-regexp-in-string 
	 ...
	 (cl-case system-type 
	   (windows-nt
	    (concat (file-name-directory (cl-first (cl-second info)))
		    (replace-regexp-in-string "\\\\" "/" (cl-second (cl-second info)))))
	   ((darwin gnu/linux)
	    (cl-first (cl-second info)))))))
  ...
  (defun gdl-devo (executable) 
    ...
    (setq gdl:*gdl-toplevel* 
	  (concat gdl:*gdl-toplevel-base*
		  (if (cl-equalp (cl-subseq (file-name-sans-extension (file-name-nondirectory executable))
				      0 1) "a")
		      "(ANSI)" "(modern)")))
