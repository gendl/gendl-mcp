...
(defmethod gdl-slots::object-keywords ((basis t) &rest args)
  (declare (ignorable args) (optimize (speed 2)(debug 3)(safety 1))))

(defmethod gdl-slots::object-keywords ((basis gdl-basis) &rest args)
  (declare (ignorable args) (optimize (speed 2)(debug 3)(safety 1))))


(defun gdl-rule::%trickle-down-try% (self)
  (let ((self-class (class-of self)))
    (or (gdl::trickle-down-cache self-class)
	(setf (gdl::trickle-down-cache self-class)
	      (let ((classes
		      (remove-if-not #'(lambda(cla) (eql (class-of cla)
							 (find-class 'gdl-class)))
		      		     (cons (class-of self)
					   (mapcar #'find-class (the all-mixins))))))
		(let* ((trickles
			 (remove-duplicates
			  (apply #'append
				 (mapcar #'(lambda(class)
		  ...
		  (dolist (trickle trickles ht)
		    (setf (gethash (make-keyword trickle) ht) t))))))))
...
toplevel of the define-object form.")
  ...
  (
   ...
   ;;(%name% :toplevel :settable) ;; :settable causes issues with expanded input methods
   (%name% :toplevel) 
   (%parent% nil)
   (%aggregate% nil)
   (%index% nil :settable)
...
sequence."  strings-for-display (format
 ...
 as flat lists. Any children which throw errors come back as a plist with error information"
  ...
  ((%corners% nil)
   ...
   (%trickle-down-slots% (gdl-rule::%trickle-down-try% self) :uncached)

   (%settable-slots% (gdl-rule::%settable-slots% self))

   #+nil
   (%object-keywords% (mapcar #'make-keyword (the object-keywords)))

   ;;
   #+nil
   (%object-keywords% (let ((unsorted
                              (mapcar #'make-keyword (gdl-rule::%object-keywords% self))))
                        (if *sort-children?* (sort unsorted #'string-lessp) unsorted)))
   ...
   ;;
   ;; Put this (maybe slow) one back for
   ;; now... new one hasn't been tested enough.
   ;;
   ;; After release we can provide hotpatches to test other variants.
   ;; Infrastructure in define-object macro remains in place to allow
   ;; switching to it for testing. Hopefully that will not noticeably
   ;; bloat the .fasl files.
   ;;
   ;; #+nil
...
the <tt>follow-root-path</tt> GDL function to return the actual instance."
  ...
  ((object-keywords
    ()
    (declare (optimize (speed 2)(debug 3)(safety 1)))
    (let* ((my-class (class-of self))
           (superclass-objects (call-next-method))
           (current-messages (messages my-class))
           (remaining-from-supers
             (set-difference superclass-objects current-messages)))
      (remove-duplicates
       (append (objects my-class) (quantified-objects my-class)
               remaining-from-supers))))
...
slots would not be affected by this function." restore-root! (&key root-paths-to-ignore)
        (let ((root-paths 0)(messages 0))
          (dolist (entry (gdl-acc::%version-tree% (the root)))
            (destructuring-bind (root-path &rest plist) entry
              (when (and plist (not (member root-path root-paths-to-ignore :test #'equalp)))
                (incf root-paths)
...
;;
;; Current one which works for production uses. This still needs to be
;; fixed because it's not respecting the global
...
;;
;; Broken one originally released with 1596:
;;
#+nil
(defun %unbind-dependent-slots% (object slot &key updating?)
  (let ((current (gethash (list object slot) *unbound-slots*)))
