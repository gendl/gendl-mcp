...
(defun local-timezone ()
  (multiple-value-bind
        (seconds minutes hours date month year day daylight-savings? timezone)
      (get-decoded-time)
    (declare (ignore seconds minutes hours date month year day daylight-savings?))
    timezone))


(defun iso-8601-date (universal-time &key include-time? (time-zone (local-timezone)))
