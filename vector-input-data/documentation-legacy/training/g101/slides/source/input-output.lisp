...
passing it to <i>format</i>")))
      ...
      (:title 
       ...
       :bullet-points
       (
        (:description "The format function is a very powerful, but rather cryptic, feature. Below are a few of the more common advanced features")
        
        
        (:description "All format directives start with a ~and ends with a single character that identifies the directive. The character can be upper or lower case. Some of the directives take prefix parameters")

        (:description "For example, the $ directive outputs a number to 2dp, but specifying a number &lt;n&gt; before it outputs the number to n dp. f on the other hand allows 2 prefix parameters, the number of dp's controlled by the second parameter. Multiple parameter are separated by commas, so, &lt;n&gt; has the same effect as &lt;n&gt;$"
	 :examples
		      ((:code (format t "~$" 19.6722998)
			:return-value 19.67)
		       (:code (format t "~5$" 19.6722998)
			:return-value 19.67229)
		       (:code (format t "~,5f" 19.6722998)
			:return-value 19.67229)))


        (:description "Directives can also be modified using the : and @ modifiers"
	 :examples
		      ((:code (format t "~d" 5000)
			:return-value 5000)
		       (:code (format t "~:d" 5000)
			:return-value 5,000)
		       (:code (format t "~@d" 5000)
			...
			:return-value +5,000)))))
      (:title 
       "Formatting English Language Directives"
       :bullet-points
       (
	(:description "R is an amazing feature, used to convert numbers to english language"
		      ((:code (format t "~r" 54502355)
			:return-value fifty-four million five hundred two thousand three hundred fifty-five)))
	(:description "P deals with plurals"
		      ((:code (format t "computer~p" 1)
		        :return-value computer)
		       (:code (format t "computer~p" 2)
			:return-value computers)))
	(:description "in most cases P is used with the : modifier, causing the processer to back up and re-use the supplied argument"
		      ((:code (format t "~a computer~:p" 1)
		        :return-value  1 computer)
		       (:code (format t "~a computer~:p" 2)
			:return-value 2 computers)))
	(:description "and with the @ modifier, <i>y</i> can be replaced by <i>ies</i>"
		      ((:code (format t "~r fl~:@p" 1)
		        :return-value  one fly)
		       (:code (format t "~r fl~:@p" 2)
		        :return-value  two flies)
		       ))))
       (:title 
	"Conditional Formatting"
	:bullet-points
	
	((:description "Wrapping a directive in ~@[ ~] causes the output t print only if it is non nil"
                       ((:code (format t "~@[~a blue computer~:p~] ~@[~a red computer~:p~]" nil 1)
                         :return-value 1 red computer)
                        (:code (format t "~@[~a blue computer~:p~] ~@[~a red computer~:p~]" 2 nil)
                         :return-value 2 blue computers)
                        (:code (format t "~@[~a blue computer~:p~] ~@[~a red computer~:p~]" 2 3)
                         :return-value 2 blue computers 3 red computers)))))

      (:title "Formatting with Itteration"
       :bullet-points
              (
               (:description "Wrapping a directive in ~{ ~} causes the processor to itterate through a list of supplied values"
			     ((:code (setq lis (list 1 2 3 4))
			       :return-value (1 2 3 4))
			      (:code (format t "~{~a-~}" lis)
			       :return-value 1-2-3-4-  )))
	       (:description "and if required the final , can be suppressed by preceeding it with ~^"
			     ((:code (format t "~{~a~^-~}" lis)
			       :return-value 1-2-3-4  )))
	       (:description "finally, if the arguments are doscrete values rather than a list, using an @ modifier causes them to be treated as a list"
			     ((:code (format t "~@{~a~^-~}" 1 2 3 4)
		       	       :return-value 1-2-3-4 )))
	       (:description "The above is just a few examples of using format. Its a very powerful function but can also be quite cryptic") ))
