...
output is coming later.")
  ...
  ;;
  ;; Below external-format and element-type values are taken from
  ;; cl-pdf/config.lisp:
  ;;
  
  :slots ((external-format
	   #+ccl :latin1
	   #+sbcl :latin-1
	   #-(or allegro ccl sbcl) :default
	   #+(and allegro mswindows) :octets
	   #+(and allegro unix) :default
	   )

	  (element-type #+(and pdf-binary sbcl) :default
			#+(and pdf-binary ccl) '(unsigned-byte 8)
			#+(and pdf-binary allegro) nil
			#-(or sbcl ccl allegro) (error "Need element-type for ~a~% "
						       (lisp-implementation-type))
			#-pdf-binary gdl:*with-format-element-type*))
