...
"
   ...
   (distance-along-axis
    (axis point &key (from (the center)) signed?)

    (let ((axis (ecase axis
		  ((:front :rear :longitudinal) :rear)
		  ((:right :left :lateral) :right)
		  ((:top :bottom :up :down :vertical) :top))))
      (let ((u-plane-1 (ecase axis
			 ((:rear :right) (the (face-normal-vector :top)))
			 (:top (the (face-normal-vector :rear)))))
	    (u-plane-2 (ecase axis
			 ((:rear :top) (the (face-normal-vector :right)))
	...
	(let ((start (inter-line-plane point u-plane-1 p-plane u-plane-1))
	  ...
	  (let ((start (inter-line-plane start u-plane-2 p-plane u-plane-2))
	    ...
	    (let ((absolute-value (3d-distance start end)))
	      (if (not signed?) absolute-value
		  (let ((start-end-vector (subtract-vectors end start))
			(positive-axis-vector (the (face-normal-vector axis))))
		    (if (same-direction-vectors? start-end-vector positive-axis-vector)
...
"
  ...
  (
   ...
   (find-closest-direction (direction &key wrt)
			   (let ((wrt-vector (theo wrt (face-normal-vector direction)))
				 theta local-direction)
			     (dolist (box-dir (list :right :left :rear :front :top :bottom))
			       (let ((self-vector (the (face-normal-vector box-dir))))
				 (let ((delta (angle-between-vectors-d wrt-vector self-vector)))

				   (when (or (null theta) (< delta theta))
				     (setq theta delta local-direction box-dir)))))
			     local-direction))
...
keywords: :right, :left, :rear, :front, :top, :bottom.\")
  ...
  (wrt \"GDL object instance. Reference object for axes\")"
    ...
    (if wrt
	(let ((axis (the (find-closest-direction direction :wrt wrt))))
	  (the (face-center axis)))
...
:arguments (direction \"Keyword. One of the standard direction keywords: :right, :left, :rear, :front, :top, :bottom.\")"
    face-normal-vector
    (direction &key wrt)
    (when wrt (setq direction (the (find-closest-direction direction :wrt wrt))))
