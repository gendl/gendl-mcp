...
(defun answer (req ent)
  (let ((query (request-query req)))
    ...
    (let* ((requestor (query-arg-to-root-path query "requestor"))
      ...
      (if (and (typep root-object 'base-html-sheet) (theo root-object lock-responses?))
	  (bt2:with-lock-held ((theo root-object response-lock))
	    (finish-answer req ent :query query :requestor requestor :iid iid :hash-entry hash-entry :root-object root-object))
	  (finish-answer req ent :query query :requestor requestor :iid iid :hash-entry hash-entry :root-object root-object)))))


(defun finish-answer (req ent &key query requestor iid hash-entry root-object)
  (let* ((skin (third hash-entry))
...
(defun publish-shared  (object-type
			&key
			  (path "/")
			  (paths (ensure-list path))  
			  (server *http-server*)
			  ...
			  content-type ;; by default this comes from object.
			  aserve-args ;; to provide extra aserve args.
			  object-args ;; to provide extra args to the object instance. 
			  )
...
:&key ((path \"/\") \"String. Must start with \"/\". The URI path to be published. Defaults to \"/\".\"
       ...
       (server *http-server*) \"Allegroserve server object. If you have additional servers other than the default
                            <tt>*http-server*</tt> (e.g. an SSL server) then you may want to call this function for each server.\"
       (host nil) \"Hostname for URI. Must be a valid hostname. By default comes from the object instance.\"
       ...
       (object nil) \"if you want to provide an already-instantiated one. Can be a list. Will override object-type.\"
       (content-type nil) \"by default comes from the object instance.\"
...
:example <pre>
  ...
  (let ((self (or object (apply #'make-object (if (stringp object-type)
    ...
    (let ((hosts (or (ensure-list (or hosts host (the host))) (list nil)))
	  (function
	    #'(lambda(req ent) (the (before-response!)) (the (present-part req ent)))))
      (dolist (path paths)
	...
	(dolist (host hosts)
	  (apply #'publish :path path
			   :server server
			   :host host
			   :content-type (or content-type (the content-type))
			   :function function aserve-args))))))
...
(define-object form-element-processor ()
  ...
  (
   ...
   (force-validation-for (when (the bashee) (the bashee force-validation-for)))
