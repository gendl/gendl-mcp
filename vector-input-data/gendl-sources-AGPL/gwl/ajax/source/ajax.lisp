...
(let ((timeout-counter nil) (lockout-lock (bt:make-lock)))
  (defun set-layout-refresh-lockout (&key (timeout 1))
    (bt:with-lock-held (lockout-lock)
      (setq timeout-counter (bt:make-thread #'(lambda()
						(sleep timeout)
						(setq timeout-counter nil))))))
  (defun get-layout-refresh-lockout () timeout-counter))
...
(define-object ajax-response ()
  ...
  ((security-ok? (the respondent root do-security-check))
   ...
   (html-section-objects (the respondent html-sections-valid))
  ...
  ((respond! 
    ...
    (let* ((replace-lists (the replace-lists))
           (layed-out-root-paths-before-after
	     (when (the respondent evaluate-html-sections?)
	       (let ((before (the respondent %html-section-root-paths%)))
		 (the respondent reset-html-sections!)
		 (the respondent (restore-slot-default! :main-sheet-body))
		 (the respondent main-sheet-body) ;; evaluate it to force setting
		 (list before (the respondent %html-section-root-paths%)))))
           (layout-same? (or (null layed-out-root-paths-before-after)
			     (destructuring-bind (before after) layed-out-root-paths-before-after
			       (let ((same? (equalp before after)))
				 (unless same? (when *debug?* (print-variables  before after)))
				 same?)))))
	  ...
	  (with-html-output (*html-stream* nil)
	    ...
	    (if (or layout-same? (get-layout-refresh-lockout))
                (htm
		 (set-layout-refresh-lockout :timeout 1)
                 (:document
	          (mapc #'(lambda(replace-pair js-to-eval-status)
		        ...
		        replace-lists (the js-to-eval-previi))))

                (htm (:document (:html-section (:|replaceId| nil)
					       (:|newHTML| nil)
					       (:|jsToEval| "location.reload(true);"))))
