...
interface. Defaults to nil."
  ...
  ((standard-javascript-section
    ...
    (with-cl-who-string (:indent t)
      (when (the use-jquery?)
	(htm 
	 ((:script :src (format nil "~a/static/3rdpty/jquery/js/jquery.min.js"
				(the proxy-prepend))))
	 ((:script :src (format nil "~a/static/3rdpty/jquery/js/jquery-ui.min.js"
				(the proxy-prepend))))
	 ((:script :src (format nil "~a/static/3rdpty/jquery/js/jquery.layout.min.js"
				(the proxy-prepend))))
	 ((:script :src (format nil "~a/static/3rdpty/jquery/js/superfish.min.js"
				(the proxy-prepend))))
	 ((:script :src (format nil "~a/static/3rdpty/jquery/js/jquery.bgiframe.js"
				(the proxy-prepend))))))
      
      (when (the use-x3dom?)
	(htm ((:script :src (if (the local-assets?)
				(format nil "~a/static/3rdpty/x3dom/x3dom.js"
					(the proxy-prepend))
				"https://www.x3dom.org/download/1.7.2/x3dom.js")
		       :id "x3dom_script"))
	     #+nil
	     ((:link :href (if (the local-assets?)
			       (format nil "~a/static/3rdpty/x3dom/x3dom.css"
				       (the proxy-prepend))
	...
	(htm ((:script
	       ...
	       :src (if (the local-assets?)
			(format nil "~a/static/3rdpty/raphael/js/raphael-min.js"
				(the proxy-prepend))
	...
	(htm ((:script
	       :id "svg-panzoom"
	       :src (if (the local-assets?)
			(format nil "~a/static/3rdpty/svgpanzoom/svg-pan-zoom.min.js"
				(the proxy-prepend))
	...
	(htm ((:link :id "fontawesome-css"
		     ...
		     :href (if (the local-assets?)
			       (format nil "~a/static/3rdpty/fa/css/all.min.css"
				       (the proxy-prepend))
	...
	(htm ((:script
	       ...
	       :src (if (the local-assets?)
			(format nil
				"~a/static/3rdpty/resize/any-resize-event.js"
				(the proxy-prepend))
			"https://is.gd/sAeEPt")))))
	...
	(htm
	 ...
	 ((:script :src
		   ;;"/static/gwl/js/gdlajax1598.js"
		   (if (the local-assets?)
		       (format nil "~a/static/gwl/js/gdlajax~a.js"
			       (the proxy-prepend) 1595)
		       (format nil "https://genworks.com/static/gwl/js/gdlajax~a.js"
			       1595))))))
  ...
  :functions ((prepend-url-depth
	      ...
	      (reset-html-sections! 
	       ()
	       (the (restore-slot-default! :%html-section-root-paths%))
	       )
  ...
  ((main-sheet
	...
	(:title (str (the title)))
        ...
        (when (the include-default-favicon?)
          (htm (:link :rel "icon" :type "image/x-icon"
		      :href (format nil "~a/static/gwl/images/favicon.ico"
				    (the proxy-prepend)))))
