...
(defparameter *display-shaded-edges?* nil)
...
to make up a solid brep."
  ...
  (
   #+nil
   (outline-objects (when (or *display-shaded-edges?* (getf (the display-controls) :shaded-edges))
                      (list-elements (the edges))))
   (hide-faces? t)
...
of this brep. Exact supported format will be documented here when ready." 
   ...
   (tessellation-lines
    ...
    ;;
    (destructuring-bind
	(&key vertex-indices 3d-points &allow-other-keys) (the %tess-data%)
      (let ((3d-points (coerce 3d-points 'vector))
            (index-triplets (make-triplets vertex-indices)))
        (mapcan 
         #'(lambda(triplet)
             (list (list (svref 3d-points (first triplet))
                         (svref 3d-points (second triplet)))
                   (list (svref 3d-points (second triplet))
                         (svref 3d-points (third triplet)))
                   (list (svref 3d-points (third triplet))
                         (svref 3d-points (first triplet))))) index-triplets))))
