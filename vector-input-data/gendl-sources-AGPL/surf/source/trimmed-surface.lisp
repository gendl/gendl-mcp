    ...
    (let ((island-3d (when (the island)
      ...
      (let ((island-uv (when (the uv-inputs) (mapcar #'(lambda (curve) (make-b-spline-curve*-2d *geometry-kernel* curve))
        ...
        (make-trimmed-face *geometry-kernel* brep 
                           ...
                           (when (the uv-inputs) (cons island-uv holes-uv))
...
(define-object trimmed-surface-2 (trimmed-surface)
  :input-slots
  ((planar? nil)
   (in-plane-tolerance *3d-tolerance-default*)
   (basis-surface nil)
   )
  ...
  (
   ...
   (%native-brep%
    (let ((island-3d (when (the island)
      ...
      (let ((island-uv (when (the uv-inputs)
                         (mapcar #'(lambda (curve) (make-b-spline-curve*-2d *geometry-kernel* curve))
                                 island-3d)))
            (holes-uv (when (the uv-inputs) 
                        (mapcar #'(lambda (hole) (mapcar #'(lambda (curve) (make-b-spline-curve*-2d *geometry-kernel* curve)) hole)) ;;hack
                                holes-3d))))

        (cond ((the holes) (error "Holes are not yet supported in trimmed-surface-2."))
              ((and (the planar?) (null (the holes)))
               (create-planar-face  *geometry-kernel* brep island-3d (the in-plane-tolerance)))
            ;; FLAG -- replace below with create-trimmed-faces
              (t (make-trimmed-face *geometry-kernel* brep 
                                    (when (not (the uv-inputs)) (cons island-3d holes-3d))
                                    (when (the uv-inputs) (cons island-uv holes-uv))
                                    (cons (if (the reverse-island?) 2 1) 
                                          (make-list (length (the holes)) :initial-element (if (the reverse-holes?) 1 2)))
   ...
   (u-iso-curves (if (the basis-surface) (the basis-surface u-iso-curves)
                     (the %extracted-basis% u-iso-curves)))
   
   (v-iso-curves (if (the basis-surface) (the basis-surface v-iso-curves)
                     (the %extracted-basis% v-iso-curves)))

   ))


;;
;; FLAG -- export later and maybe use in full-blown trimmed-surface. 
;; 
(defun chain-curves (curves)
  (flet ((rectify (start-or-end curve)
	   (ecase start-or-end
	     (:start curve)
	     (:end (theo curve reverse)))))
    (let* ((curve (first curves))
	   (end (theo curve end))
	   (rest (rest curves))
	   (neighbor-plist (dolist (candidate rest)
			     (cond ((coincident-point? end (theo candidate start))
				    (return (list :start candidate)))
				   ((coincident-point? end (theo candidate end))
				    (return (list :end candidate)))
				   (t nil)))))

      (cons curve
	    (when neighbor-plist
	      (chain-curves (let ((neighbor (apply #'rectify neighbor-plist)))
			      (cons neighbor (remove (second neighbor-plist) rest)))))))))
