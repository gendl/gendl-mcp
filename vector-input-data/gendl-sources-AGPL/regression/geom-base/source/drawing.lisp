...
(define-object box-with-annotated-drawing (base-object)
  ...
  :computed-slots ((truncate-lines 8)

                   (regression-test-data
                    (let ((lines (glisp:split-regexp
                                  "\\n" 
                                  (glisp:replace-regexp
                                   (glisp:replace-regexp
                                    (glisp:replace-regexp
                                     (glisp:replace-regexp
                                      (the drawing-pdf-string)
                                      "\\/ModDate (.*)" "")
                                     "\\/CreationDate (.*)" "")
                                    "\\s+[-0\\.]+0\\s" " 0.0")
                                   "Length.*" "Len_"))))
                      (subseq lines 0 (- (length lines) (the truncate-lines)))))

                     
                   (drawing-pdf-string
                    (let ((file (glisp:temporary-file)))
                      (with-format (pdf file) (write-the drawing cad-output))
                      (prog1
...
(register-test-definition 'box-with-annotated-drawing)
