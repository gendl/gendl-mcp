...
;;
;; FLAG -- maybe need to add crlf-base-ef for Allegro Windows
;;
(defparameter *with-format-external-format* glisp:*external-text-format*
  ...
  for the base format. Defaults to gdl::*external-text-format*.")

(defparameter *with-format-element-type* glisp:*text-element-type*)

;;
;; FLAG -- determine what this should really be, i.e. establish a glisp:*element-type-default*
;;
#+nil
(defparameter *with-format-element-type*
  #+allegro nil
  #+(and pdf-binary sbcl) :default
  #+(and pdf-binary (not sbcl)) '(unsigned-byte 8)
  #+(and (not pdf-binary) (not allegro)) 'base-char
  #-(or allegro sbcl ccl)
...
(defparameter *with-format-element-type*
  #+allegro nil
  #+pdf-binary #+sbcl :default #+ccl '(unsigned-byte 8)
  #-pdf-binary 'base-char
  #-(or allegro sbcl ccl) (error "*with-format-element-type* needs a value for ~a.~%"
				 (lisp-implementation-type))
"Element-type. The default for the :element-type format-slot
  for the base format. Defaults to nil. Needs a better default.")
