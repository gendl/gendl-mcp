...
;;
;; Some predefinitions in GWL needed by Gendl init
;;
(in-package :gwl)

(eval-when (:compile-toplevel :load-toplevel :execute)
  (defvar *http-port* (let ((value (uiop:getenv "HTTP_PORT"))) (if value (parse-integer value)  9080)))
  (defvar *https-port* (let ((value (uiop:getenv "HTTPS_PORT"))) (if value (parse-integer value)  9443)))
  (defvar *aserve-listeners* 25)
  (defvar *aserve-start-args* nil)
  (defvar *publishers* nil)
...
(defmacro with-all-servers ((server) &rest body)
  (let ((server-symbol (gensym)))
    `(dolist (,server-symbol (list '*http-server* '*https-server*))
       (unless (symbol-value ,server-symbol)
         (setf (symbol-value ,server-symbol)
               (make-instance (read-from-string "net.aserve:wserver")
                              :enable-compression (and (member :zlib-deflate *features*) t))))
       (let ((,server (symbol-value ,server-symbol))) ,@body))))

#+nil
(defmacro with-all-servers ((server) &rest body)
  (let ((server-var (gensym)))
    `(dolist (,server-var (list *http-server* *https-server*))
       (when ,server-var
         (let ((,server ,server-var)) ,@body)))))

(defun publish-uris ()
  (format t "~&Publishing standard endpoints...~%")
