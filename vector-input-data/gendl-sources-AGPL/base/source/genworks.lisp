...
(defparameter *external-text-format*
  #+(and allegro windows) (excl:crlf-base-ef :default)
  #-(and allegro windows) nil)



(defparameter *text-element-type*
  #+allegro nil
  #-allegro 'base-char
  #-(or allegro sbcl ccl)
  (error "*with-format-element-type* needs a value for ~a.~%"
	 (lisp-implementation-type))
"Element-type. The default for the :element-type format-slot
  for the base format. Defaults to nil for allegro or 'base-char for
  non-allegro.")
...
(defun executable-homedir-pathname ()
  ...
  (let* ((exe (first (glisp:basic-command-line-arguments)))
    ...
    (make-pathname :name nil :type nil
                   :defaults     (if (eql (first exe-dir) :relative)
                                     (merge-pathnames exe (uiop/os:getcwd))
                                     exe))))
...
(defun set-default-float-format ()
  ...
  #+sbcl (setf (sb-ext:symbol-global-value '*read-default-float-format*) 'double-float)
...
(defun set-settings (settings)
  (let (anything-changed?)
    ...
    ;;(gdl:print-variables settings)
    
    (dolist (setting settings)

      ;;(gdl:print-variables setting)
      
      (destructuring-bind (symbol default new-value-or-func) setting

	;;(gdl:print-variables symbol default new-value-or-func)
	
	(let ((new-value (or default
			     (if (functionp new-value-or-func)
...
;;#+sbcl
#+nil
(defun find-definition-pathname (object)
...
(defmethod ccl:toplevel-function :before ((a gendl:gendl-development-system) init-file)
  ...
  (unless (uiop:getenv "EMACS_GDL") (gendl::initialize :startup-banner? t))
  (when (fboundp 'gdl::*restart-init-function*)
    (funcall #'gdl::*restart-init-function*)))
