    ...
    (loop
      ...
      (let* ((current (pop directories-to-process))
        ...
        (dolist (entry entries)
          (if (glisp:file-directory-p entry)
...
(defun copy-directory (from-dir to-dir &rest args)
  ...
  ;;
  ;; From Quicklisp, bundle.lisp
  ;;
  #+sbcl
  (progn
    (setf from-dir (truename from-dir))
    (map-directory-tree
     from-dir
     (lambda (from-pathname)
       (when (probe-file from-pathname)
         (let* ((relative (enough-namestring from-pathname from-dir))
                (relative-directory (pathname-directory relative))
                (to-pathname (merge-pathnames relative to-dir)))
...
#+(or allegro lispworks clozure sbcl)
(defun copy-file (from to)
  #+allegro (sys:copy-file from to :overwrite t)
  #+lispworks (lw:copy-file from to)
  #+clozure (ccl:copy-file from to)
  #+sbcl (uiop:copy-file from to))
...
are no longer supported by glisp:match-regexp.~%"))
  ...
  #+allegro (let ((result (multiple-value-list
			   (excl:match-regexp string-or-regexp string-to-match :start start :end end :return :index))))
	      (when (and result (consp result) (first result))
		(values (first (second result)) (rest (second result)))))
...
#+linux
(defun getuid ()
  #-(or allegro ccl sbcl) (error "need getuid impl.")
  (#+allegro excl.osi:getuid #+ccl ccl::getuid #+sbcl sb-posix:getuid))

#+linux
(defun setuid (num)
    #-(or allegro ccl sbcl) (error "need setuid impl.")
    (#+allegro excl.osi:setuid #+ccl ccl::setuid #+sbcl sb-posix:setuid num))

#+linux
(defun setgid (num)
    #-(or allegro ccl sbcl) (error "need setgid impl.")
    (#+allegro excl.osi:setgid #+ccl ccl::setgid #+sbcl sb-posix:setgid num))

  
(defun send-letter (host from to subject body &key cc bcc reply-to headers attachments
						(port 25) ssl authentication)
  #+(or sbcl ccl)
  (if (find-package :cl-smtp)
      (funcall (read-from-string "cl-smtp:send-email")
	       host from to subject body
	       :cc cc :bcc bcc :reply-to reply-to
	       :extra-headers headers :attachments attachments
	       ;;
	       ;; FLAG - port 587 with `:ssl :starttls` and
	       ;; `:authentication '("<login>" "<password>")` is not
	       ;; working -- tested with smtppro.zoho.com.
	       ;;
	       ;; getting a cl+ssl error.
	       ;;  
    
	       ;; 
	       ;;
	       :port port :ssl ssl :authentication authentication)
      (warn "Send-letter attempted but cl-smtp is not
loaded.  You may want to `(ql:quickload :cl-smtp)` or otherwise add
cl-smtp to your dependencies.~%"))
  #+allegro
  (progn

    (let* ((starttls? (case ssl (:starttls t)
			   (nil nil)
			   (otherwise (error "Only ssl of nil or :starttls are currently supported."))))

	    (host (if (consp host)
		    (error "host must be a string. Use :port and :ssl as separate key arguments")
		    (list host :port port :starttls starttls?))))
	  
      (destructuring-bind (login password) (or authentication (list nil nil))

	(require :smtp)
	(funcall (read-from-string "net.post-office:send-letter")
		 host from to body
		 :subject subject
		 :cc cc :bcc bcc :reply-to reply-to
		 :headers headers :attachments attachments :login login
		 :password password)))))
